...
public class JarIndexer {
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			System.out.println(
					"Application usage: <program> <version> <base path 1> [<base path 2> ... [<base path N>]] \n"); 
			System.exit(1);
		}
	...
	private static void processJarDirectory(final String version, final List<String> basePaths) {
		...
		try {
			...
			for (int i = 0; i < basePaths.size(); i++) {
				...
				JavaUtils.printToConsole("Scanning directory \"" + 
						basePaths.get(i) + "\" for JAR files...");
				
				Collection<File> jarFiles = FileUtils.listFiles(new File(basePaths.get(i)), 
						new String[] {"jar"}, true);
	...
	private static void saveIndeces(final String version, final List<String> basePaths,
			final List<JarInfo> jarNames, final List<PackageInfo> packageNames, 
			final List<RawClassInfo> classNames) throws IOException {
		
		JavaUtils.printToConsole("Saving index to file ...");
		FileWriter fileWritter = null;
		try {
			fileWritter = new FileWriter(JavaUtils.getPathToFileInCurrentDirectory(
					GlobalSettings.INDEX_FILE_NAME));
			JarIndexRenderer.writeHeader(fileWritter, version, basePaths.size(), 
				jarNames.size(), packageNames.size(), classNames.size());
	...
	private static void sortIndeces(List<JarInfo> jarNames,
			List<PackageInfo> packageNames, List<RawClassInfo> classNames)
			throws Exception {
		...
		for (int i = 0; i < classNames.size(); i++) {
			...
			if (jarIndex < 0) {
				throw new Exception("Unknown jar \"" + classInfo.jarInfo.name + 
						"\" for class \"" + classInfo.name + "\"");
			}
			classInfo.jarFileIndex = jarIndex;
			
			int packageIndex = Collections.binarySearch(packageNames, classInfo.packageInfo);
			if (packageIndex < 0) {
				throw new Exception("Unknown package \"" + classInfo.packageInfo.name + 
						"\" for class \"" + classInfo.name + "\"");
			}
	...
	private static void processClasses(final String basePath,
			final int basePathIndex, Collection<File> jarFiles, List<JarInfo> jarNames,
			List<PackageInfo> packageNames, List<RawClassInfo> classNames)
			throws IOException {
